...
## Unity 2022.3 LTS での配置・動作確認ポイント
- このリポジトリを Unity プロジェクト直下にそのまま配置すると `Assets/Editor/DomToCanvas/` が Editor 専用アセンブリとしてインポートされ、
  ランタイムビルドには含まれません（フォルダ名が `Editor` のため）。
- 依存パッケージは標準の `UnityEngine`, `UnityEditor`, `UnityEngine.UI` のみで、追加インストールは不要です。
- TextMeshPro が導入されている環境では自動的に `TextMeshProUGUI` を利用し、未導入の場合は標準 `UI.Text` へフォールバックするため、
  TextMeshPro の有無に関わらずコンパイルが通ります。
- 実際の確認手順（Unity 2022.3 LTS）
  1. プロジェクトを開くとスクリプトが自動コンパイルされ、コンソールにエラーが無いことを確認。
  2. メニュー **Tools ▶ DomToCanvas ▶ Import UI JSON...** が表示されることを確認。
  3. `Samples/sample_calendar_ui.json` を指定してインポートし、シーン上に Canvas/Panel/Text/Button などが生成されることを確認。
